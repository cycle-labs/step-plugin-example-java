...
@StepIdentifier("assignCounterSecret")
@Component
public class AssignCounterSecretStep implements StepDefinition {

    private static final Logger logger = LoggerFactory.getLogger(AssignCounterSecretStep.class);

    @Override
    public StepResponse call(SuiteContext suiteContext, ScenarioContext scenarioContext, StepInputs inputs) {
        logger.info("Executing assignCounterSecret step");

        // Get the counter.
        int counter = scenarioContext.getScenarioState().get(CounterConstants.KEY_COUNTER);
        logger.debug("Current counter value: {}", counter);
        logger.debug("Assigning counter to SECRET_VARIABLE");

        // Return the step response with the assigned variable.
        return new StepResponse()
            .status(ExecutionStatus.PASS)
            .putVariablesItem(CounterConstants.SECRET_VARIABLE, counter)
            .message("Assigned the counter value of '" + counter +
                "' to variable '" + CounterConstants.SECRET_VARIABLE + "'.");
    }
